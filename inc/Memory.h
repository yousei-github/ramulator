...
#if (MEMORY_USE_HYBRID == DISABLE)
    long max_address;
#endif
    MapScheme mapping_scheme;

public:
#if (USER_CODES == ENABLE) && (MEMORY_USE_HYBRID == ENABLE)
    long max_address;
#endif
    enum class Type
    {
    ...
    void finish(void)
    {
        ...
        Stats::statlist.update_read_requests(num_read_requests[0].value());
        Stats::statlist.update_write_requests(num_write_requests[0].value());
#endif
    }
